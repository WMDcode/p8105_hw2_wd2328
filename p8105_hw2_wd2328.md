...
There are 173 ZIP codes included in the ZIP code data-set but not the
Zillow data-set. These include:
...
Note: There is one particular ZIP code (11693) that shows up in both
data-sets, but the entry in the ZIP code data-set has it mistakenly
listed under Kings County (Brooklyn), whereas the Zillow data-set has it
listed under Queens County (Queens), which is correct. Therefore, the
row/observation containing ZIP code 11693 and Kings County could be
removed.
...
to give an accurate estimate of the ZORI for that time period. When the
